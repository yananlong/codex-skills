...
6. Run the planned passes using the active chat model or Codex subagents when authorized. Each pass writes one JSON array under `comments/`.
7. Consolidate findings into `final_issues.json`, `review_summary.json`, and `overall_assessment.txt` using the normal schema.
8. Run `python3 scripts/validate_review_bundle.py --review-dir <review_dir>` before treating the bundle as stable.
9. Optionally run `python3 scripts/save_viz_json.py <review_dir> --slug-suffix _skill` if the upstream engine is installed and visualization JSON is useful.
