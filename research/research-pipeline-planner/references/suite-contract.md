...
| result validity audit | `research-results-auditor` | result artifact, target claim, protocol, run lineage, gate result, failure ledger |
...
6. Record material observations, failures, and scope changes.
7. Checkpoint before risky, expensive, destructive, or interruptible operations.
8. Write an episode package and request `approve`, `revise`, or `block`.
9. Do not present self-review as independent verification. When the same actor label verifies its own work, pass `--self-review` and disclose the limitation.
...
The harness contract governs scheduling, state, retries, checkpoints, experiment activation, and transition authority; the review-stage contract governs substantive review artifact shape.
